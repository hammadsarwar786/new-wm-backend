...
const add = async (req, res) => {
  ...
  // }
  console.log(req.body);
  const { leadId, agentName, agentId, mangerName, managerId } =
    req.body;
    ...
    try {
      ...
      const {leadPhoneNumber,leadEmail,nationality,interest,leadName} = lead;
      const newApproval = new AdminApproval({
        leadName:leadName,
        leadId,
        agentName,
...
const get = async (req, res) => {
  try {
    ...
    const { approvalStatus, page = 1, pageSize = 10,agentId,managerId } = req.query;
    ...
    if (approvalStatus) {
      filters.push({ approvalStatus: { $regex: new RegExp(`^${approvalStatus.trim()}$`, 'i') } });
    }
