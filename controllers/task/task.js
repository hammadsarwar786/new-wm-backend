...
const add = async (req, res) => {
    try {
        ...
        const result = new Task(taskData);
         ...
         const updateUser = await User.findById(result?.createBy);
        if(updateUser) {
            updateUser.tasksCreated = updateUser.tasksCreated + 1; 
...
const deleteMany = async (req, res) => {
    try {
        ...
        // if(updateUser) {
        //     updateUser.tasksCreated = updateUser.tasksCreated - (req.body?.length | 0); 
        //     await updateUser.save(); 
        // }
