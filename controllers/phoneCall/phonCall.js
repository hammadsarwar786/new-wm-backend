...
const history = async (req, res) => {
    try {
        ...
        const allCalls = await PhoneCall.find(query).sort({startDate: 1}).populate({
            path: "sender"
        }).exec(); 
        
        res.json({lead: {createdAt: lead?.createdDate, createdBy: lead._doc.leadName}, calls: allCalls}); 
    } catch (err) {
