...
const add = async (req, res) => {
  try {
    ...
    const updateUser = await User.findById(user?.createBy).populate({
      path: "roles",
    }).exec();
    if (updateUser) {
      updateUser.propertiesCreated = updateUser.propertiesCreated + 1;
      updateUser.totalRevenue += parseFloat(req.body?.listingPrice) || 0;
      await updateUser.save();
      if (updateUser?.roles[0]?.roleName === "Agent") {
        const parent = await User.findById(updateUser?.parent?.toString());
        parent.totalRevenue += parseFloat(req.body?.listingPrice) || 0;
        await parent.save();
      }
    }
    await user.save();
...
const addMany = async (req, res) => {
  try {
    ...
    // if(updateUser) {
    //     updateUser.propertiesCreated = updateUser.propertiesCreated + (data?.length | 0);
    //     await updateUser.save();
    // }
    const totalPrice = req.body
      ?.map((p) => parseFloat(p?.listingPrice))
      ?.reduce((a, b) => a + b);
    const user = await User.findById(req.user.userId);
    user.totalRevenue += totalPrice;
    await user.save();
    if (user?.roles[0]?.roleName === "Agent") {
      const parent = await User.findById(user?.parent?.toString());
      parent.totalRevenue += parseFloat(totalPrice) || 0;
      await parent.save();
    }
...
const edit = async (req, res) => {
  try {
    ...
    const oldPrice = parseFloat(result?._doc?.listingPrice) || 0;
    const newPrice = parseFloat(req.body?.listingPrice) || 0;

    const diff = newPrice - oldPrice;

    const user = await User.findById(result?._doc?.createBy?.toString());
    user.totalRevenue += diff;
    await user.save();

    if (user?.roles[0]?.roleName === "Agent") {
      const parent = await User.findById(user?.parent?.toString());
      parent.totalRevenue += parseFloat(diff) || 0;
      await parent.save();
    }
...
const deleteData = async (req, res) => {
  try {
    ...
    const updateUser = await User.findById(property?.createBy);

    if (updateUser) {
      updateUser.propertiesCreated = updateUser.propertiesCreated - 1;
      updateUser.totalRevenue -= Number(property?.listingPrice);
      await updateUser.save();

      if (updateUser?.roles[0]?.roleName === "Agent") {
        const parent = await User.findById(updateUser?.parent?.toString());
        parent.totalRevenue -= parseFloat(property?.listingPrice) || 0;
        await parent.save();
      }
    }
...
const deleteMany = async (req, res) => {
  try {
    ...
    const propertiesToBeDeleted = await Property.find({
      _id: { $in: req.body },
    });
    const totalPrice = propertiesToBeDeleted
      ?.map((d) => Number(d?._doc?.listingPrice) || 0)
      ?.reduce((a, b) => a + b);
    const user = await User.findById(req.user.userId);
    user.totalRevenue -= totalPrice;
    await user.save();
